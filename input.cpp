...
bool input_from_term(equation_info *eq) {
    LOGGER_INFO("Input from term started");
    ...
    if (scanf("%lf %lf %lf", &eq->a, &eq->b, &eq->c) != 3 || 
        ...
        !isfinite(eq->b) ||
        !isfinite(eq->c)) {
        discard_line_and_check_from_term();
        soft_assert_functional(false, "Incorrect input", LOGGER_ERROR("Incorrect input"); return false);
    }
    if (!discard_line_and_check_from_term()) {
        soft_assert_functional(false, "Incorrect input", LOGGER_ERROR("Incorrect_input");return false);
    }
    return true;
}
 ...
 * @return Возвращает корректный ли ввод
*/
bool input_from_buffer(equation_info *eq, const char *curr_file) { //TODO: ДОбавить hard assert functional
